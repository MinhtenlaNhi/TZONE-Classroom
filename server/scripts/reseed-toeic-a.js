/**
 * Cập nhật lộ trình bài học cho TẤT CẢ khóa học TOEIC A (slug danh mục: "toeic-a"),
 * áp dụng cho cả khóa cũ lẫn mới.
 *
 ...
 * trong server/src/data/toeicACurriculum.js.
 *
 * ⚠️  Thao tác này ghi đè — bài học/bài tập cũ của các khóa TOEIC A sẽ bị thay thế.
 *
 * Cách chạy (trong thư mục server/):
 *   node scripts/reseed-toeic-a.js --dry-run   -> chỉ liệt kê, không ghi
 *   node scripts/reseed-toeic-a.js             -> thực hiện cập nhật
 */
...
async function run() {
  ...
  const cat = await Category.findOne({ slug: "toeic-a" }).select("_id name slug").lean();
  if (!cat) {
    console.error("❌ Không tìm thấy danh mục slug 'toeic-a'.");
    await mongoose.disconnect();
    process.exit(1);
  }

  const courses = await Course.find({ categoryRef: cat._id }).select("_id title categoryRef").lean();
  console.log(`📚 Tìm thấy ${courses.length} khóa thuộc danh mục TOEIC A.\n`);
  ...
  console.log("\n════════════════════════════════════════════════");
  console.log("📊 KẾT QUẢ CẬP NHẬT TOEIC A:");
