...
export default function AdminCoursesPage() {
  ...
  const handleDelete = async (id, title, enrollmentCount = 0) => {
    if (enrollmentCount >= 1) {
      toast.error(`Không thể xóa khóa học vì đang có ${enrollmentCount} học viên đang học.`);
      return;
    }
                  ...
                  <tr key={course._id}>
                    ...
                    <td className="text-center">
                      <div className="tz-course-actions">
                        ...
                        <button
                          className="tz-btn-icon-circle tz-bg-red"
                          title={
                            (course.enrollmentCount || 0) >= 1
                              ? `Không thể xóa — đang có ${course.enrollmentCount} học viên`
                              : "Xóa"
                          }
                          disabled={(course.enrollmentCount || 0) >= 1}
                          onClick={() => handleDelete(course._id, course.title, course.enrollmentCount || 0)}
                        >
